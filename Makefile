...
IMAGE_TAG_BASE ?= ${REG}/${USR}/example.com/memcached-operator
...
run: ansible-operator ## Run against the configured OpenShift cluster in ~/.kube/config
	ANSIBLE_ROLES_PATH="$(ANSIBLE_ROLES_PATH):$(shell pwd)/roles" $(ANSIBLE_OPERATOR) run

podman-login: ## Login to container registry
	sudo podman login ${REG} -u ${USR}
...
install: kustomize ## Install CRDs into the OpenShift cluster specified in ~/.kube/config.
	$(KUSTOMIZE) build config/crd | oc apply -f -

uninstall: kustomize ## Uninstall CRDs from the OpenShift cluster specified in ~/.kube/config.
	$(KUSTOMIZE) build config/crd | oc delete -f -

deploy: kustomize ## Deploy controller to the OpenShift cluster specified in ~/.kube/config.
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default | oc apply -f -

undeploy: ## Undeploy controller from the OpenShift cluster specified in ~/.kube/config.
...
bundle-build: ## Build the bundle image.
	sudo podman build -f bundle.Dockerfile -t $(BUNDLE_IMG) .
